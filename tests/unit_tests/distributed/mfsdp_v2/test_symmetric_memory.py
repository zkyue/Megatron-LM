...
def test_fully_shard_symmetric_memory_matches_default_and_profiles_nccl(
    ...
    assert all("ncclSymk" not in event.name for event in allgather_kernels_without_symm_mem)
    assert all("ncclSymk" not in event.name for event in reduce_scatter_kernels_without_symm_mem)
    ...
    assert (
        len(reduce_scatter_kernels_with_symm_mem) == expected_reduce_scatter_kernel_count
    ), (
        "Unexpected NCCL symmetric-memory reduce-scatter kernel count. "
        f"Observed reduce-scatter kernels: "
        f"{[event.name for event in reduce_scatter_kernels_with_symm_mem[:20]]}"
    )
    assert all("ncclSymk" in event.name for event in reduce_scatter_kernels_with_symm_mem), (
    ...
    assert len(allgather_kernels_with_symm_mem) == expected_allgather_kernel_count, (
        ...
        f"{[event.name for event in allgather_kernels_with_symm_mem[:20]]}"
    )
    assert all("ncclSymk" in event.name for event in allgather_kernels_with_symm_mem), (
...
def test_fully_shard_zero_cta_moves_all_gather_to_copy_engine(distributed_setup):
    ...
    assert not allgather_kernels, (
        ...
        f"Observed all-gather kernels: {[event.name for event in allgather_kernels[:20]]}"
    )
    ...
    assert len(reduce_scatter_kernels) == expected_reduce_scatter_kernel_count, (
        ...
        f"{[event.name for event in reduce_scatter_kernels[:20]]}"
    )
    assert all("ncclSymk" in event.name for event in reduce_scatter_kernels), (
        "Expected all zero-CTA reduce-scatter kernels to be ncclSymk kernels. "
        f"Observed reduce-scatter kernels: {[event.name for event in reduce_scatter_kernels[:20]]}"
    )
