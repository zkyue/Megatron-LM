...
def validate_encoder_prefetch_args(args) -> None:
    ...
    if args.encoder_tp != 1:
        raise ValueError("encoder prefetch requires encoder TP=1")
...
def _log_consumer_debug(batch_id: int, ready: int, depth: int, wait_start: float) -> None:
    logger.info(
        "encoder-prefetch-debug consumer batch=%d ready=%d/%d pop_wait_ms=%.3f",
        batch_id,
        ready,
        depth,
...
class EncoderPrefetchLoader:
    ...
    def __next__(self) -> dict[str, object]:
        ...
        if self._debug:
            ...
            _log_consumer_debug(batch_id, ready, self._depth, wait_start)
        return item
